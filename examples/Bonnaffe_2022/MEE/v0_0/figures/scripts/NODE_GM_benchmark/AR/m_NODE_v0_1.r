...
## model.Ybar_DE
## goal:
model.Ybar_DE = function(times, Y_0, Beta)
{
  Ybar = NULL
  Y = Y_0
  for(t in times)
...
## model.Ybar_ODE
## goal:
model.Ybar_ODE = function(times, Y_0, Beta)
{
...
{
  TS_pred = model.Ybar(times=TS[,1], Y_0=Y_0, Beta=Beta)
...
model.predict = function(Theta)
{
  Y_0  = exp(Theta[idx_Y_0])
  Beta = Theta[idx_Beta]
  Ybar = model.Ybar(times=TS[,1], Y_0=Y_0, Beta=Beta)
  return(Ybar)
}

## model.dYdt
model.dYdt = model.dYdt_NODE
model.Ybar = model.Ybar_ODE
n_Beta = n_Beta_NODE
